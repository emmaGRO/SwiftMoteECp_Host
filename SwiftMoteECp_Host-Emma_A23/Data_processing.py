...
def extract_gains(voltages: list[float], currents:list[float]) -> dict:
    voltages = voltages[50:-1]
    currents = currents[50:-1]
    ...
    try:
        ...
        max_gain_index1 = normalized_gain.index(np.max(normalized_gain))
        max_gain_index = data_smooth.index(np.max(data_smooth))
        print("MAX_GAIN_INDEX", max_gain_index)
        print("MAX_GAIN_INDEX1", max_gain_index1)
        ...
        return {"gain coefs":gain_coeff,
                ...
                "peak current1": currents[max_gain_index1],
