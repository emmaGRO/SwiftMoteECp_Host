...
//
int runSWV(uint8_t lmpGain, int16_t startV, int16_t endV,
...
//    SerialDebugger.println(F("Time(ms),Zero,LMP,V1,Time(ms),Zero,LMP,V1,Voltage,Current"));
    ...
    if (startV < endV){
      SerialDebugger.println("running SWV forward");
      runSWVForward(startV, endV, pulseAmp, stepV, ms,saveData);  
    }
    else if (startV > endV){
      SerialDebugger.println("running SWV backward");
      runSWVBackward(startV, endV, pulseAmp, stepV, ms,saveData);  
    }
    else{
      SerialDebugger.println("Error in input");
    }
...
int runSWVForward(int16_t startV, int16_t endV, int16_t pulseAmp,
...
int runSWVBackward(int16_t startV, int16_t endV, int16_t pulseAmp,
                    int16_t stepV, double freq,bool saveData)
{
  float i_forward = 0;
  float i_backward = 0;
...
//    SerialDebugger.print((String(realTime)));
//    SerialDebugger.print(F(" ms,"));
    //positive pulse
    i_forward = biasAndSample(j + pulseAmp,freq);
